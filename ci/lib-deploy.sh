...
rsync_to_server() {
  ...
  if ! rsync -azv --delete -i "${SSH_KEY}" -o "StrictHostKeyChecking=no" -o "IdentitiesOnly=yes" "${LOCAL_PATH}" "${SSH_CONNECT}:${SERVER_PATH}"; then
...
deploy_docker_swarm() {
  ...
  copy_file_to_server "${DOCKER_COMPOSE_FILE}" "/srv/${CI_PROJECT_NAME}/${DOCKER_COMPOSE_FILE}"

  # Run Docker Stack Deploy.
  if ! ssh_run_command docker stack deploy -c "/srv/${CI_PROJECT_NAME}/${DOCKER_COMPOSE_FILE}" --with-registry-auth "${STACK_NAME}"; then
...
deploy_helm_chart() {
  ...
  if [[ -z "${CI_ENVIRONMENT_NAME}" ]]; then
    error "Gitlab CI Variables not defined: 'CI_ENVIRONMENT_NAME'!"
  fi
  ...
  local CHART_DIR=`basename "${CHART_URL}"`

  # Setting SSH requirements.
  set_ssh

  # Remove helm chart on server.
  if ! ssh_run_command rm -rf "/srv/${CHART_DIR}"; then
    error "Remove Helm Chart directory failed!"
  fi

  # Download helm chart to server.
  if ! ssh_run_command helm pull "${CHART_URL}" --version "${CHART_VERSION}" --untar --untardir "/srv"; then
  ...
  for GITLAB_VARIABLE in "CI_ENVIRONMENT_NAME" "CI_ENVIRONMENT_NAME_LOWER"; do
    sed -ie "s#\${${GITLAB_VARIABLE}}#${!GITLAB_VARIABLE}#g" "${VALUES_FILE}"

    if [[ -n "${GLOBAL_VALUES_FILE}" ]]; then
      sed -ie "s#\${${GITLAB_VARIABLE}}#${!GITLAB_VARIABLE}#g" "${GLOBAL_VALUES_FILE}"
    fi
  done
  ...
  for GITLAB_VARIABLE in `printenv | awk -F "=" '/^DC_/ {print $1}'`; do
    ...
    if [[ -f "${!GITLAB_VARIABLE}" ]]; then
      ...
      # Copy created file to server.
      copy_file_to_server "${FILENAME}" "/srv/${CHART_DIR}/${FILENAME}"
  ...
  # Copy values file.
  copy_file_to_server "${VALUES_FILE}" "/srv/${CHART_DIR}/${VALUES_FILE}"

  # Copy global values file if exists.
  if [[ -n "${GLOBAL_VALUES_FILE}" ]]; then
    copy_file_to_server "${GLOBAL_VALUES_FILE}" "/srv/${CHART_DIR}/${GLOBAL_VALUES_FILE}"
  fi

  # Install/upgrade Helm Chart without global values.
  if [[ -z "${GLOBAL_VALUES_FILE}" ]]; then
    if ! ssh_run_command helm upgrade --install -f "/srv/${CHART_DIR}/${VALUES_FILE}" "${CHART_DIR}" "/srv/${CHART_DIR}"; then
      error "Helm Chart install/upgrade failed!"
    fi

  # Install/upgrade Helm Chart with global values.
  else
    if ! ssh_run_command helm upgrade --install -f "/srv/${CHART_DIR}/${GLOBAL_VALUES_FILE}" -f "/srv/${CHART_DIR}/${VALUES_FILE}" "${CHART_DIR}" "/srv/${CHART_DIR}"; then
      error "Helm Chart install/upgrade failed!"
    fi
  fi
}
